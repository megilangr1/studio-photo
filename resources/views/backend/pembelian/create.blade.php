...
<div class="row">
  <div class="col-12">
    <div class="card card-outline card-primary">
      ...
      <div class="card-body">
        <form action="{{ route('backend.pembelian.store') }}" method="post">
          @csrf
          <div class="row">
            <div class="col-md-3">
              <div class="form-group">
                <label for="tanggal_pembelian">Tanggal Pembelian : </label>
                <input type="date" name="tanggal_pembelian" id="tanggal_pembelian" class="form-control {{ $errors->has('tanggal_pembelian') ? 'is-invalid':'' }}" value="{{ old('tanggal_pembelian') }}" placeholder="Masukan Tanggal..." required autofocus>
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <input type="string" name="nomor_kwitansi_pembelian" id="nomor_kwitansi_pembelian" class="form-control {{ $errors->has('nomor_kwitansi_pembelian') ? 'is-invalid':'' }}" value="{{ old('nomor_kwitansi_pembelian') }}" placeholder="Masukan Nomor Kwitansi..." required>
                <span class="invalid-feedback">
              ...
              <div class="form-group">
                ...
                <select name="studio_id" id="studio_id" class="form-control {{ $errors->has('studio_id') ? 'is-invalid':''}}" style="width: 100%;" data-placeholder="Silahkan Pilih Studio" required>
                  ...
                  @foreach ($studio as $item)
                    <option value="{{ $item->id }}" {{ old('studio_id') == $item->id ? 'selected':'' }}>{{ $item->nama_studio }}</option>
                  @endforeach
                </select>
