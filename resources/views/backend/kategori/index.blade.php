  ...
  <div class="col-12">
    <div class="card card-outline card-primary">
      <div class="card-header">
        <h5 class="card-title"> <span class="fa fa-box text-primary"></span> &ensp; Data Kategori</h5>
        <div class="card-tools">
          <a href="{{ route('backend.kategori.create') }}" class="btn btn-xs btn-primary px-2">
              ...
              <table class="table table-bordered m-0">
                ...
                <tbody>
                  @forelse ($kategoris as $item)
                    <tr>
                      <td class="align-middle text-center">{{ ($kategoris->currentpage()-1) * $kategoris->perpage() + $loop->index + 1 }}.</td>
                      <td class="align-middle">{{ $item->nama_kategori }}</td>
                      <td class="align-middle">{{ $item->keterangan ?? 'Tidak Ada Keterangan' }}</td>
                      <td class="align-middle text-center">
                        <div class="btn-group">
                          <a href="{{ route('backend.kategori.edit', $item->id) }}" class="btn btn-sm btn-warning borad">
